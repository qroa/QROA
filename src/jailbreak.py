...
class JailBreak:
    def __init__(self, device, config):
        ...
        self.logging_generator_path = os.path.join(
            self.logging_path,
            "logging_generator.json"
        )
        self.logging_validator_path = os.path.join(
            self.logging_path,
            "logging_validator.json"
        )

        self.logging_generator_path_csv = os.path.join(
            self.logging_path,
            "logging_generator.csv"
        )
        self.logging_validator_path_csv = os.path.join(
            self.logging_path,
            "logging_validator.csv"
        )

        self.triggers_path = os.path.join(
            self.results_path,
            "triggers.json"
        )
        self.triggers_validate_path = os.path.join(
            self.results_path,
            "triggers_validate.json"
        )
    ...
    def _read_csv(self, file_path):
        """
        Reads a CSV file into a DataFrame. Returns an empty DataFrame 
        if the file is empty or only contains headers.
        """
        try:
            if os.path.exists(file_path):
    ...
    def run(self, instructions):
        ...
        triggers["universal"] = self.trigger_generator.run(instructions)

        logging_generator = pd.concat(
            [logging_generator, self.trigger_generator.return_logging()],
            ignore_index=True
        )
        ...
        triggers_validate["universal"] = self.trigger_validator.run(
            instructions, triggers["universal"]
        )

        logging_validator = pd.concat(
            [logging_validator, self.trigger_validator.return_logging()],
            ignore_index=True
        )
